...
sub COVERAGE {
  die(qq/
Usage Assembly_evaluation.pl COVERAGE [options]

Options: -p FILE pileup file generated by samtools mpileup -f ref.fa in.bam > in.pileup 
         -c INT coverage cutoff [5] 
         -q INT base quality cutoff [13] 
         -o FILE outfile name
\n/) if (!@ARGV);
  
  my %opts = (p=>undef, c=>5, q=>13, o=>undef);
  getopts('p:c:q:o:', \%opts);
...
my @pline = split(/\s+/, <PILEUP>);
  ...
  while (<PILEUP>) {
    ...
    } elsif ($prev_cont ne $contig) { # new contig
      $avg_depth = $depth/$pos;
      printf OUT "Average Coverage: %.3f", $avg_depth;
      print OUT "\n";
      printf  "Average Coverage: %.3f", $avg_depth;
...
sub FIX {
  die(qq/
Usage Assembly_evaluation.pl FIX [options]
